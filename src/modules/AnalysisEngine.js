...
class AnalysisEngine {
  ...
  analyze(dependencyGraph) {
    ...
    const report = {
      summary: {
        ...
        unusedPropsCount: unusedProps.reduce((total, comp) => total + comp.unusedProps.length, 0),
    ...
    for (const [nodeId, node] of nodes) {
      ...
      if (unusedProps.length > 0) {
        unusedPropsList.push({
          ...
          usageRate: (node.propsUsedInBody.size / node.propsDeclared.size) * 100
        });
      }
    }
    
    return unusedPropsList.sort((a, b) => b.unusedProps.length - a.unusedProps.length);
  }
  ...
  analyzeDependencyDepth(nodes) {
    ...
    // 统计信息
    const avgDepth = maxDepths.reduce((sum, d) => sum + d, 0) / maxDepths.length;
    const maxDepth = Math.max(...maxDepths);
    
    // 找出深度最大的组件
    const deepestComponents = [];
    for (const [nodeId, depth] of depthMap) {
      if (depth >= maxDepth * 0.8) { // 深度超过80%最大值的组件
        const node = nodes.get(nodeId);
  ...
  analyzeCoupling(nodes, edges) {
    ...
    return {
      ...
      highCouplingComponents: highCouplingComponents.sort((a, b) => b.totalCoupling - a.totalCoupling),
  ...
  generateRecommendations(analysisData) {
    ...
    if (analysisData.orphanComponents.length > 0) {
      const isolatedCount = analysisData.orphanComponents.filter(c => c.reason === 'isolated').length;
      if (isolatedCount > 0) {
        recommendations.push({
          ...
          components: analysisData.orphanComponents.filter(c => c.reason === 'isolated').slice(0, 5)
        });
      }
    }
    ...
    if (analysisData.unusedProps.length > 0) {
      const totalUnusedProps = analysisData.unusedProps.reduce((sum, c) => sum + c.unusedProps.length, 0);
      recommendations.push({
        type: 'unused_props',
        priority: 'medium',
        title: '清理未使用的Props',
    ...
    if (analysisData.couplingAnalysis.highCouplingComponents.length > 0) {
      const veryHighCouplingCount = analysisData.couplingAnalysis.highCouplingComponents
        .filter(c => c.category === 'very_high').length;
      ...
      if (veryHighCouplingCount > 0) {
        recommendations.push({
          ...
          components: analysisData.couplingAnalysis.highCouplingComponents
            .filter(c => c.category === 'very_high').slice(0, 5)
        });
      }
    }
