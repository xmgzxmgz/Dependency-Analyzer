...
describe('Logger 单元测试', function() {
  ...
  describe('构造函数', function() {
    it('应该正确初始化配置', function() {
      ...
      expect(logger.config.logDir).to.equal('/test/logs');
      expect(logger.config.maxFileSize).to.equal(1024 * 1024);
    ...
    it('应该使用默认配置', function() {
      ...
      expect(defaultLogger.config.enableConsole).to.be.true;
      expect(defaultLogger.config.enableFile).to.be.true;
    });

    it('应该创建日志目录', function() {
      fs.existsSync.returns(false);
      new Logger({ logDir: '/test/new-logs' });
      expect(fs.mkdirSync.calledWith('/test/new-logs', { recursive: true })).to.be.true;
    });
  });
