...
describe('CacheManager', () => {
  ...
  describe('统一缓存接口', () => {
    it('应该优先使用内存缓存', async () => {
      ...
      cacheManager.setToMemory('test-key', memoryData);
    ...
    it('应该回退到文件缓存', async () => {
      ...
      // 验证数据已回填到内存缓存
      const memoryResult = cacheManager.getFromMemory('test-key');
    ...
    it('应该同时存储到内存和文件', async () => {
      ...
      const memoryResult = cacheManager.getFromMemory('test-key');
      const fileResult = await cacheManager.getFromFile('test-key');
  ...
  describe('缓存管理', () => {
    it('应该删除缓存', async () => {
      ...
      // 验证存在
      expect(await cacheManager.get('test-key')).to.deep.equal(testData);
      
      // 删除
      await cacheManager.delete('test-key');
      
      // 验证已删除
      expect(await cacheManager.get('test-key')).to.be.null;
    });
    ...
    it('应该清理过期缓存', async () => {
      ...
      // 验证过期文件已被删除
      const cacheFile = path.join(cacheManager.cacheDir, 
