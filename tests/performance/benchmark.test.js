...
export default ${componentName};
    ...
    const indexCode = `
import React from 'react';
${components.slice(0, 10).map(c => `import ${c.name} from './${c.name}';`).join('\n')}

const App = () => {
  return (
    <div>
      ${components.slice(0, 10).map(c => `<${c.name} title="Test" data="Sample" />`).join('\n      ')}
    </div>
  );
};
...
export default App;
  ...
  describe('小型项目性能 (10个组件)', () => {
    it('应该在1秒内完成分析', async function() {
      ...
      console.log(`内存使用: ${(report.memory.delta.heapUsed / 1024 / 1024).toFixed(2)}MB`);
    });
  });
