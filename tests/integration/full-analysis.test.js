...
export default App;
    ...
    await fs.writeFile(path.join(testProjectDir, 'OrphanComponent.jsx'), orphanComponent);
  ...
  describe('React项目分析', () => {
    it('应该成功分析React项目', async () => {
      ...
      // 验证输出文件
      expect(await fs.pathExists(path.join(outputDir, 'react-analysis.html'))).to.be.true;
      expect(await fs.pathExists(path.join(outputDir, 'react-data.json'))).to.be.true;

      // 验证JSON数据结构
      const jsonData = await fs.readJson(path.join(outputDir, 'react-data.json'));
    ...
    it('应该检测到循环依赖', async () => {
      ...
      const result = await analyzer.analyze();
      const jsonData = await fs.readJson(path.join(outputDir, 'circular-analysis.html').replace('.html', '.json'));
  ...
  describe('排除模式', () => {
    it('应该正确排除指定文件', async () => {
      ...
      const result = await analyzer.analyze();
      const jsonData = await fs.readJson(path.join(outputDir, 'exclude-analysis.html').replace('.html', '.json'));
  ...
  describe('输出验证', () => {
    it('HTML报告应该包含必要元素', async () => {
      ...
      const htmlContent = await fs.readFile(path.join(outputDir, 'html-validation.html'), 'utf8');
    ...
    it('JSON数据应该有正确的结构', async () => {
      ...
      const jsonData = await fs.readJson(path.join(outputDir, 'json-validation.json'));
