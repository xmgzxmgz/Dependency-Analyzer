...
describe('CLI 端到端测试', function() {
  ...
  beforeEach(function() {
    // 创建临时测试目录
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dependency-analyzer-test-'));
...
`;
  ...
  describe('项目分析功能', function() {
    it('应该成功分析测试项目', function() {
      ...
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}"`);
      
      expect(result.success).to.be.true;
      expect(result.output).to.include('分析完成');
      
      // 检查输出文件是否生成
      expect(fs.existsSync(path.join(outputDir, 'analysis-report.json'))).to.be.true;
      expect(fs.existsSync(path.join(outputDir, 'dependency-graph.json'))).to.be.true;
    });
    ...
    it('应该检测孤立组件', function() {
      ...
      expect(report.orphanComponents.some(file => file.includes('OrphanComponent'))).to.be.true;
    });
    ...
    it('应该检测未使用的属性', function() {
      ...
      expect(report.unusedProps.some(item => 
        item.unusedProps && item.unusedProps.includes('unused')
      )).to.be.true;
    });
  ...
  describe('输出格式选项', function() {
    it('应该支持JSON格式输出', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --format json`);
    ...
    it('应该支持HTML格式输出', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --format html`);
  ...
  describe('排除模式功能', function() {
    it('应该支持排除特定文件模式', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --exclude "**/*Orphan*"`);
    ...
    it('应该支持多个排除模式', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --exclude "**/*Orphan*,**/*Component2*"`);
  ...
  describe('详细输出模式', function() {
    it('应该在详细模式下显示更多信息', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --verbose`);
    ...
    it('应该在静默模式下减少输出', function() {
      ...
      const outputDir = path.join(tempDir, 'output');
      const result = runCLI(`--project "${testProjectDir}" --output "${outputDir}" --quiet`);
  ...
  describe('配置文件支持', function() {
    it('应该支持从配置文件读取选项', function() {
      ...
      expect(result.success).to.be.true;
      expect(fs.existsSync(path.join(tempDir, 'config-output', 'analysis-report.json'))).to.be.true;
    });
  });

  describe('错误处理', function() {
    it('应该处理无效的输出目录', function() {
      const result = runCLI(`--project "${testProjectDir}" --output "/invalid/readonly/path"`);
